...
def rover_reward(obs, act):
    ...
    if obs[0][1] < 40:
        # reward if head is located in very red region... nutrient rich
        reward += 1.0
    if obs[0][1] > 225 and obs[0][1] < 255:
        reward += -1.0
...
grad_field = Grad_field(LRED, DRED, size[0], size[1], border)
...
while epi < max_epis:
    ...
    while running:
        ...
        # end episode if positive reward is achieved
        if reward == 1:
            running = False
        elif reward == -1:
            running = False
        ...
        if running == False:
            ...
            if epi % batch_size == 0:
                print('\nbatch ended with batch reward %f --> back prop!\n' % batch_reward)
